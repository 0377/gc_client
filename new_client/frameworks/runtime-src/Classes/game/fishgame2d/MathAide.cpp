...
void MathAide::BuildLinear(float initX[], float initY[], int initCount, std::vector<MyPoint>& TraceVector, float fDistance)
{
	TraceVector.clear();
	...
	float cosValue = fabs(initY[initCount - 1] - initY[0]) / disTotal;
	float angle = acosf(cosValue);
...
void MathAide::BuildBezier(float initX[], float initY[], int initCount, MovePoints& TraceVector, float fDistance)
{
...
void MathAide::BuildCircle(float centerX, float centerY, float radius, MovePoints& FishPos, int FishCount)
{
...
void MathAide::BuildCirclePath(float centerX, float centerY, float radius, MovePoints& FishPos, float begin, float fAngle, int nStep, float fAdd)
{
	if(fAngle == 0.0f || radius == 0) return;
	if(nStep < 1) nStep = 1;

	int nCir = 2 * M_PI * radius / nStep;
	int nCount = nCir * fabs(fAngle) / (2 * M_PI);
	float cell_radian = 2 * M_PI / nCir * fAngle / fabs(fAngle);
...
MyPoint MathAide::GetRotationPosByOffest(float xPos, float yPos, float xOffest, float yOffest, float dir, float fHScale, float fVScale)
{
	MyPoint pt;

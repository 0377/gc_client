...
def main():
    ...
    try:
        ...
        tolua_root = '%s/tools/tolua/CustomBindingFiles' % project_root
        output_dir = '%s/../runtime-src/Classes/lua-bridge' % project_root

        cmd_args = {
                    #'NetworkManager.ini' : ('NetworkManager', 'lua_NetworkManager_auto'), \
                    'LuaBridgeUtils.ini' : ('LuaBridgeUtils', 'lua_LuaBridgeUtils_auto'), \
                    # 'HLCustomRichText.ini' : ('HLCustomRichText', 'lua_HLCustomRichText_auto'), \
                    #####MyDownloader is manual ,can't auto generate\
                    # 'MyDownloader.ini' : ('MyDownloader', 'lua_MyDownloader_manual'), \
					# 'game.fishgame2d.ini' : ('game_fishgame2d', 'lua_game_fishgame2d_auto'), \
					# 'XMLParser.ini' : ('XMLParser', 'lua_XMLParser_auto')
                    }
        target = 'lua'

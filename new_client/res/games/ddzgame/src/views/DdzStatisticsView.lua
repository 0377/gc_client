    ...
    for i = 1, table.getn(data) do
        if (chairs[i]) then
            ...
            if (itemData >= 0) then
            	...
            	item:getChildByName("Text_Item_Win"):setString(string.format("+%s", tostring(itemData)))
            else
            	-- item:getChildByName("Text_Item_Lose"):setColor(cc.c3b(0xe2, 0x0b, 0x13))
            	item:getChildByName("Text_Item_Lose"):setString(string.format("%s", tostring(itemData)))
            end
        end
    end
end

...
--------------------------------------------------------------------------
local SHGameDataBaseManager = class("SHGameDataBaseManager")

local SHConfig = import("..cfg.SHConfig")
local SHHelper = import("..cfg.SHHelper")
function SHGameDataBaseManager:ctor()
...
function SHGameDataBaseManager:setGameScene(SHGameNormalScene)
	self.SHGameNormalScene = SHGameNormalScene
end
---保存进入游戏
...
function SHGameDataBaseManager:OnMsg_EnterRoomAndSitDownInfo(msgTab)
...
----
function SHGameDataBaseManager:setGameRoomInfo()
    -- body
    local pb_gmMessage = {}
...
--	//reconnect
--};
function SHGameDataBaseManager:on_SC_ShowHand_Desk_Enter(msgTab)
...
--}
function SHGameDataBaseManager:on_SC_ShowHand_Desk_State(msgTab)
...
function SHGameDataBaseManager:on_SC_ShowHand_Game_Finish(msgTab)
...
--	repeated ShowHand_Player_Info pb_players 	= 1; 		// 玩家
--}
function SHGameDataBaseManager:on_SC_ShowHand_Next_Round(msgTab)
	--这里处理 得到一张牌
...
--  optional int32 chair_id	    = 2; 	// id
--}
function SHGameDataBaseManager:on_SC_ShowHandAddScore(msgTab)
	ssdump(msgTab," 玩家下注")
	local opType = nil	
...
--}
function SHGameDataBaseManager:on_SC_ShowHandGiveUp(msgTab)
	ssdump(msgTab," 玩家下注")
	local cardOperation = {
...
--  optional int32 chair_id	    = 1; 	// id
--}
function SHGameDataBaseManager:on_SC_ShowHandPass(msgTab)
	ssdump(msgTab," 玩家让牌")
	local cardOperation = {
...
--	//16 --弃牌
--}
function SHGameDataBaseManager:on_SC_ShowHand_NextTurn(msgTab)
	ssdump(msgTab," 玩家发言")
...
function SHGameDataBaseManager:on_SC_ChatTable(msgTab)
...
--返回非自己的座位ID
function SHGameDataBaseManager:getNotSelfChairId()
...
--根据用户ID获取座位ID
function SHGameDataBaseManager:getChairIdById(guid)
	local ret = nil

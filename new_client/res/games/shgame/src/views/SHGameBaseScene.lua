...
--------------------------------------------------------------------------
local SHGameBaseScene = class("SHGameBaseScene",requireForGameLuaFile("SubGameBaseScene"))
...
function SHGameBaseScene.getNeedPreloadResArray()
	local resNeed = {
		CustomHelper.getFullPath("game_res/animation/sh_pxdh_eff.ExportJson"),
	}
	return resNeed
end
function SHGameBaseScene:ctor()
	SHGameBaseScene.super.ctor(self)
...
----注册消息
function SHGameBaseScene:registerNotification()
    SHGameBaseScene.super.registerNotification(self)
...
function SHGameBaseScene:receiveServerResponseSuccessEvent(event)
    ...
    SHGameBaseScene.super.receiveServerResponseSuccessEvent(self,event)
end


function SHGameBaseScene:onEnter()
...
--开启设备显示信息计时器
function SHGameBaseScene:startDeviceSchedule()
	--进来就要显示时间的
...
--电池和wifi的计时器
function SHGameBaseScene:_intervalWIFI(dt)
...
--停止时间计时器
function SHGameBaseScene:stopDeviceSchedule()
...
function SHGameBaseScene:onExit()
	sslog(self.logTag,"二人梭哈onExit")
end

function SHGameBaseScene:onEnterTransitionFinish()
	sslog(self.logTag,"二人梭哈onEnterTransitionFinish")
end

function SHGameBaseScene:onExitTransitionStart()
...
function SHGameBaseScene:playerOperationHandler(pType,operationType)
	...
	local cardOperations = SHGameDataBaseManager.cardOperations
...
--循环执行玩家操作消息
function SHGameBaseScene:loopMsgOperation()
	if not self.operationComlete then
		sslog(self.logTag,"等待上一个操作完成")
		return
	end
	local SHGameDataBaseManager = SHGameManager:getInstance():getDataManager()
	local selfCharId = SHGameDataBaseManager.selfChairId
	local cardOperations = SHGameDataBaseManager.cardOperations
	local decisionTime = SHGameDataBaseManager.chooseTime
...
function SHGameBaseScene:changeTotalBet(event)
...
---重新连接成功
function SHGameBaseScene:callbackWhenReloginAndGetPlayerInfoFinished(event)
    -- body
    SHGameBaseScene.super.callbackWhenReloginAndGetPlayerInfoFinished(self,event)

end
--初始化桌子
function SHGameBaseScene:initTable()
...
function SHGameBaseScene:addOnePlayer(playerInfo)
...
--定时刷新时间
function SHGameBaseScene:_intervalTime(dt)
...
--@key name 桌子名字
--@key isAdd 底池是否显示添加的
function SHGameBaseScene:setTableInfo(tinfo)
	local fnodeTable = CustomHelper.seekNodeByName(self.tableNode,"FileNode_tableinfo")
...
--@key time 格式化时间
--@key electric 电量 0-100 表示当前电量 <0 表示正在充电中
function SHGameBaseScene:setDeviceInfo(deviceInfo)
	--fnodeDevice
	local fnodeDevice = CustomHelper.seekNodeByName(self.tableNode,"FileNode_device")
...
--关闭倒计时
function SHGameBaseScene:closeCountDown()
	local fnodeCountDown = CustomHelper.seekNodeByName(self.tableNode,"FileNode_countdown")
...
function SHGameBaseScene:setCountDown(limitTime)
	--FileNode_countdown
...
--初始化菜单按钮
function SHGameBaseScene:initMenu()
	--FileNode_menu
	local fnodeMenu = CustomHelper.seekNodeByName(self.tableNode,"FileNode_menu")
...
function SHGameBaseScene:menuListener(ref,eventType)
	if not ref then
		return
	end
	...
	elseif eventType == ccui.TouchEventType.ended then
		...
		elseif ref:getName() == "Button_quit" then --退出游戏
			--isInGame
			local SHGameDataBaseManager = SHGameManager:getInstance():getDataManager()
			if SHGameDataBaseManager.isInGame then --在游戏中才弹出提示
		...
		elseif ref:getName() == "Button_ctype" then --牌型
			--自己手上的牌要传递进入
			local SHGameDataBaseManager = SHGameManager:getInstance():getDataManager()
			local selfChairId = SHGameDataBaseManager.selfChairId
			local playerdatas = SHGameDataBaseManager.playerdatas
			local handCards = nil
...
function SHGameBaseScene:showMusicAndSoundInfoView()
	local fnodeMenu = CustomHelper.seekNodeByName(self.tableNode,"FileNode_menu")
...
function SHGameBaseScene:onHallMsgSC_StandUpAndExitRoom(msgTab)
	sslog(self.logTag,"二人梭哈 站起离开了")
	
end

function SHGameBaseScene:onHallMsgSC_EnterRoomAndSitDown(msgTab)
...
function SHGameBaseScene:onHallMsgSC_NotifySitDown(msgTab)
...
function SHGameBaseScene:onHallMsgSC_NotifyStandUp(msgTab)
	sslog(self.logTag,"二人梭哈 通知站起")

end
function SHGameBaseScene:onHallMsgSC_Ready(msgTab)
	sslog(self.logTag,"二人梭哈 准备OK")
	--ssdump(msgTab,self.logTag)
end
function SHGameBaseScene:onHallMsgSC_NotifyMoney()
	sslog(self.logTag,"我的玩家金币变动")
	local myPlayerInfo = GameManager:getInstance():getHallManager():getPlayerInfo()
	local SHGameDataBaseManager = SHGameManager:getInstance():getDataManager()
	local selfCharId = SHGameDataBaseManager.selfChairId --我自己的座位号
...
--进入游戏开局消息
function SHGameBaseScene:onMsgSC_ShowHand_Desk_Enter()
	local SHGameDataBaseManager = SHGameManager:getInstance():getDataManager()
	if not SHGameDataBaseManager.valideStart then --判断是否非法开局
		--非法开局，直接发送准备消息
	...
	local selfCharId = SHGameDataBaseManager.selfChairId --我自己的座位号
	local zhuangId = SHGameDataBaseManager.zhuangId --庄家的ID
	local isReconnect = SHGameDataBaseManager.isReconnect --是否重连
	local chooseTime = SHGameDataBaseManager.chooseTime --选择时间
	local reconnData = SHGameDataBaseManager.reconnData --重连数据
	local baseScore = SHGameDataBaseManager.baseScore --底注
	local maxCall = SHGameDataBaseManager.maxCall --限注 倍数
	local totalbet = 0
	--@headInfo 头像信息
	--@key gold  用户金币
	--@key username  用户昵称
...
--桌子状态
function SHGameBaseScene:onMsgSC_ShowHand_Desk_State()

end
--结算消息
function SHGameBaseScene:onMsgSC_ShowHand_Game_Finish()
	sslog(self.logTag,"UI结算消息")
	self:loopMsgOperation()
end
--翻牌  下一回合 发牌消息
function SHGameBaseScene:onMsgSC_ShowHand_Next_Round()
	sslog(self.logTag,"UI摸牌消息")
	self:loopMsgOperation()
end
--加注=倍数*底注，allin = -1，跟注 = 0
function SHGameBaseScene:onMsgSC_ShowHandAddScore()
	sslog(self.logTag,"UI加注，跟注，梭哈消息")
	self:loopMsgOperation()
end
--弃牌
function SHGameBaseScene:onMsgSC_ShowHandGiveUp()
	sslog(self.logTag,"UI弃牌消息")
	self:loopMsgOperation()
end
--让牌
function SHGameBaseScene:onMsgSC_ShowHandPass()
	sslog(self.logTag,"UI让牌消息")
	self:loopMsgOperation()
end
--更新发言者
function SHGameBaseScene:onMsgSC_ShowHand_NextTurn()
	sslog(self.logTag,"UI轮到谁发言消息")
	self:loopMsgOperation()
end
--玩家聊天返回
function SHGameBaseScene:onMsgSC_ChatTable(msgTab)
...
function SHGameBaseScene:onMsgSC_ReconnectionPlay(msgTab)
...
--删除玩家的倒计时动画
function SHGameBaseScene:removePlayerProgress()
	if self.seats then
		table.walk(self.seats,function (SHPlayer,seatId)
...
--结算循环队列调用
function SHGameBaseScene:gameOver()
	...
	local gameOverDatas = SHGameDataBaseManager.gameOverDatas
	local selfCharId = SHGameDataBaseManager.selfChairId
	local otherCharId = SHGameDataBaseManager:getNotSelfChairId()
...
--显示结算界面
function SHGameBaseScene:showResultLayer()

end
--删除所有的玩家
function SHGameBaseScene:deletePlayer(seatId)
...
--删除所有的玩家
function SHGameBaseScene:removeAllPlayer()
	if not self.seats then
		return
	end
...
function SHGameBaseScene:showGameOverTips()
   CustomHelper.showAlertView(
...
---退出游戏界面
function SHGameBaseScene:exitGame(openSecondLayer)
	self:stopDeviceSchedule()
end
--下一局
function SHGameBaseScene:nextGame()
	--退出游戏的时候 清空游戏数据

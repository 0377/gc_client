...
QZNNPoker = class("QZNNPoker", function()
...
--构造
function QZNNPoker:ctor(font, num)
	self:setScale(0.65)
	self._num = num --牌数
...
--得到牌型
function QZNNPoker:getCard(font, num)
	if font then
...
--显示牌型
function QZNNPoker:showTexture()
...
--翻牌动画
function QZNNPoker:openAction()
	local time = 0.2
    local function unReversal()
        self:loadTexture("QZNNPoker/poker_"..self._num..".png")
...
function QZNNPoker:getCardShowPath(issmall)
	...
	local x = gameQznn.CardsV[pointx+1]
...
function QZNNPoker:getNum()
	return self._num
	
end

--点数 大于10就等于10
function QZNNPoker:getCardPoint()
...
--真实点数
function QZNNPoker:getTrueCardPoint()
	
	return math.floor(self._num/4)+1
end


function QZNNPoker:setNum(num)
	self._num = num
...
function QZNNPoker:setCanTouch(b)
...
--[[function QZNNPoker:heng()
	self.:setRotation(90)
end--]]
--[[
--添加动画
function QZNNPoker:addAnimation(num)
...
--手动看牌动画
function QZNNPoker:flipCardAction()
    local x, y = self:getPosition()
    --local t_str = "QZNNPoker/smallpoker_"..self._num..".png"

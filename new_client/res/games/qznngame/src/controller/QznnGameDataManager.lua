QznnGameDataManager = class("QznnGameDataManager");
...
QznnGameDataManager.instance = nil

function QznnGameDataManager:getInstance()
	if QznnGameDataManager.instance == nil then
		QznnGameDataManager.instance = QznnGameDataManager:create();
	end
	return QznnGameDataManager.instance;
end

QznnGameDataManager.BankerInfo = 
{
    guid = 0,
    nickname = 0,   --昵称
    money = 0,      ---当前金币数
...
function QznnGameDataManager:ctor()

	print("QznnGameDataManager:ctor")
	-- body
...
---保存进入游戏
function QznnGameDataManager:OnMsg_EnterRoomAndSitDownInfo(msgTab)
	self._enterRoomAndSitDownInfo = msgTab
end
function QznnGameDataManager:getRoomInfo()
	return 	self._enterRoomAndSitDownInfo
end

function QznnGameDataManager:OnSC_BankerTableMatching(msgTab)
	print( "//匹配")
	self.isMatch = true
	
end

function QznnGameDataManager:on_SC_BankerSendCards(msgTab)
...
--]]
function QznnGameDataManager:on_SC_BankerBeginToContend(msgTab)
	print( "//开始抢庄")
	
	self._tableinfo.countdown = msgTab.countdown
	self._tableinfo.total_time = msgTab.total_time
	self._tableinfo.state = QznnGameManager.TexasStatus.STATUS_CONTEND_BANKER
end


function QznnGameDataManager:on_SC_BankerPlayerContend(msgTab)
...
function QznnGameDataManager:on_SC_BankerChoosingBanker(msgTab)
	
	print( "//定庄")
	self._tableinfo.state = QznnGameManager.TexasStatus.STATUS_DICISION_BANKER
...
function QznnGameDataManager:on_SC_BankerPlayerBeginToBet(msgTab)
	-- body
	print( "//闲家开始下注")
	self._tableinfo.countdown = msgTab.countdown
	self._tableinfo.total_time = msgTab.total_time
	self._tableinfo.state = QznnGameManager.TexasStatus.STATUS_BET
	
end

function QznnGameDataManager:on_SC_BankerPlayerBet(msgTab)
	-- body
	print( "//闲家下注")
	local player = self:getUserInfoByChair(msgTab.chair)
	if player ~= nil then
...
function QznnGameDataManager:on_SC_BankerShowOwnCards(msgTab)
	-- body
	print( "//玩家看到自己的牌")
	self._tableinfo.state = QznnGameManager.TexasStatus.STATUS_SHOW_CARD
	self._tableinfo.countdown = msgTab.countdown
	self._tableinfo.total_time = msgTab.total_time
...
function QznnGameDataManager:on_SC_BankerShowCards(msgTab)
	-- body
...
function QznnGameDataManager:on_SC_BankerGameEnd(msgTab)
	print( "//结算")
	...
	self._tableinfo.state = QznnGameManager.TexasStatus.STATUS_SHOW_DOWN
end

function QznnGameDataManager:on_SC_BankerForceToLeave(msgTab)
	print( "//强制离开")
end
function QznnGameDataManager:on_SC_BankerReconnectInfo(msgTab)
...
function QznnGameDataManager:getTableInfo()
	
	return self._tableinfo
	
end
function QznnGameDataManager:getUserInfoList()
	
	return self._userList
	
end

function QznnGameDataManager:getUserInfoByChair(chair)
...
--
--获取自己信息
function QznnGameDataManager:getMyInfo()
...
--获取庄家信息
function QznnGameDataManager:getZhuangJiaInfo()
